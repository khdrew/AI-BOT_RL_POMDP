...
public class RLWorld extends JPanel implements Runnable, KeyListener{
	...
	private void init(){
		...
		// sets individual state rewards
		rewardsState = new int[stateCount];
		int count = 0;
		for (int i = 0; i < grid.length; i++) {
			for (int j = 0; j < grid[0].length; j++) {
				if (grid[i][j] == 1) {
					rewardsState[count] = -100;
				}else if (grid[i][j] == 2) {
					rewardsState[count] = 100;
				} else {
					rewardsState[count] = 0;
				}
				count++;
			}
		}
	...
	public void setNextState(int action){
	...
	public int RState(int state){
		return rewardsState[state];
	}
